...
pub fn solve(capacity_1: u8, capacity_2: u8, goal: u8, start_bucket: &Bucket) -> Option<BucketStats> {
    if goal > capacity_1.max(capacity_2) || goal % gcd(capacity_1, capacity_2) != 0 {
        return None;
    }
    ...
    loop {
        ...
        for state in current_search.drain(0..) {
            ...
            let pour_2_into_1 = if bucket_1 + bucket_2 <= capacity_1 {
                (bucket_1 + bucket_2, 0)
            } else {
                (capacity_1, bucket_2 - (capacity_1 - bucket_1))
            };
            ...
            let pour_1_into_2 = if bucket_1 + bucket_2 <= capacity_2 {
                (0, bucket_1 + bucket_2)
            } else {
                (bucket_1 - (capacity_2 - bucket_2), capacity_2)
            };
